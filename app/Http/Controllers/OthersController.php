...
class OthersController extends Controller
{
    public function pageView(){
        ...
        return view('backend.add_others',compact('category'));
    }
    public function addOthers(Request $request){
        if($request->hasFile('image')){
            ...
            Image::make($get_image)->resize(800,800)->save(base_path('public/others_images/'.$image_name,100));
            ...
            Others::insert([
                ...
                'price'          => $request->price,
                'stock'          => $request->stock,
                'image'          => 'others_images/'.$image_name,
        ...
        return back()->with('msg','A New Product has been added');
    }
    public function viewOthers(){
        $others = DB::table('others')
                ...
                ->select('others.*','categories.category_name','sub_categories.subcategory_name')
                ->orderBy('id','desc')
                ->paginate(10);

        return view('backend.view_others',compact('others'));
    }
    public function deleteOthers($id){
        $image_data = Others::findOrfail($id)->image;
    ...
    public function updatePriceStock(Request $request){
        foreach($request->other_id as $value){
            Others::findOrFail($value)->update([
    ...
    public function searchOthers(Request $request){
        if($request->ajax())
        {
            ...
            $products=DB::table('others')
                            ->where('others.name','LIKE','%'.$request->search."%")
                            ->OrderBy('id','desc')
                            ->get();
            ...
            if($products)
            {
                ...
                foreach ($products as $key => $product) {
                    ...
                    '<td>'.$product->stock.'</td>'.
                    '<td>'.$product->price.'</td>'.
                    '<td>'.'<a class="btn btn-sm btn-danger rounded mb-1" href="/delete/others/'.$product->id.'">'.'Delete'.'</a>'.'</td>'.
    ...
    public function addDataByModel(Request $request){
        Others::findOrFail($request->modal_id)->update([
