        ...
        <div class="col-md-12">
            <div class="card mt-3">
                <div class="card-header bg-info text-white rounded">
                    <div class="row">
                        <div class="col-md-4">
                            <h5><i class="fas fa-server"></i> View All MotherBoards</h5>
                        </div>
                        ...
                        <div class="col-md-4 text-right">
                            <input type="text" id="search" name="search" class="form-control rounded" placeholder="Search MotherBoard">
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    <table class="table table-striped">
                        ...
                        <tbody>
                        <?php $sl = 1 ?>
                        <form action="{{url('motherboard/price/stock/update')}}" method="POST">
                            @csrf
                            <div class="row">
                                <div class="col-md-10">
                                    {{ $motherboards->links() }}
                        ...
                        <input type="hidden" name="motherboard_id[]" value="{{$motherboard->id}}">
                            <tr>
                                ...
                                <td><img src="{{url($motherboard->image)}}" style="height:50px;width:60px"></td>
                                <td>{{$motherboard->id}}</td>
                                <td>{{$motherboard->name}}</td>
                                <td>{{$motherboard->socket}}</td>
                                @if($motherboard->stock < 3)
                                <td><input min="0" type="number" placeholder="Stock" name="stock[{{$motherboard->id}}]" class="text-center form-control" value="{{$motherboard->stock}}" style="color:red;font-weight:500" required></td>
                                @else
                                <td><input min="0" type="number" placeholder="Stock" name="stock[{{$motherboard->id}}]" class="text-center form-control" value="{{$motherboard->stock}}" style="color:green;font-weight:500" required></td>
                                @endif
                                <td><input min="0" type="number" placeholder="Price" name="price[{{$motherboard->id}}]" class="text-center form-control" value="{{$motherboard->price}}" required></td>
                                <td>
                                    <a href="{{url('delete/motherboard')}}/{{$motherboard->id}}" class="btn btn-sm btn-danger rounded mb-1"><i class="fas fa-trash-alt"></i></a>
                                </td>
                                <td><a class="edit btn btn-warning btn-sm editProduct rounded" href="javascript:void(0)" data-toggle="tooltip" data-id="{{$motherboard->id}}" data-original-title="Edit"><i class="far fa-edit"></i></a></td>
                            </tr>
                            <?php $sl++ ?>
                        @endforeach
                      </form>
...
<script type="text/javascript">
    ...
    $(function () {
      ...
      $('body').on('click', '.editProduct', function () {
        ...
        $.get("{{ url('/model/get/motherboard/data') }}" +'/'+ product_id , function (data) {

            $('#modelHeading').html("Edit MotherBoard Info");
        ...
        $('#saveBtn').click(function (e) {
            ...
            url: "{{ url('/add/motherboard/from/modal') }}",
            type: "POST",
            dataType: 'json',
