            ...
            <div class="card mt-3">
                ...
                <div class="card-body">
                    <table class="table table-striped">
                        ...
                        <tbody>
                        <?php $sl = 1 ?>
                        <form action="{{url('gpu/price/stock/update')}}" method="POST">
                            @csrf
                            <div class="row">
                                <div class="col-md-10">
                                    {{ $gpus->links() }}
                        ...
                        <input type="hidden" name="gpu_id[]" value="{{$gpu->id}}">
                            <tr>
                                <th scope="row">{{$sl}}</th>
                                <td>{{$gpu->brand}}</td>
                                <td><img src="{{url($gpu->image)}}" style="height:50px;width:60px"></td>
                                <td>{{$gpu->id}}</td>
                                <td>{{$gpu->name}}</td>
                                <td>{{$gpu->watt}} watt</td>
                                @if($gpu->stock < 3)
                                <td><input min="0" type="number" placeholder="Stock" name="stock[{{$gpu->id}}]" class="text-center form-control" value="{{$gpu->stock}}" style="color:red;font-weight:500" required></td>
                                @else
                                <td><input min="0" type="number" placeholder="Stock" name="stock[{{$gpu->id}}]" class="text-center form-control" value="{{$gpu->stock}}" style="color:green;font-weight:500" required></td>
                                @endif
                                <td><input min="0" type="number" placeholder="Price" name="price[{{$gpu->id}}]" class="text-center form-control" value="{{$gpu->price}}" required></td>
                                <td>
                                    <a href="{{url('delete/gpu')}}/{{$gpu->id}}" class="btn btn-sm btn-danger rounded mb-1"><i class="fas fa-trash-alt"></i></a>
                                </td>
                                <td><a class="edit btn btn-warning btn-sm editProduct rounded" href="javascript:void(0)" data-toggle="tooltip" data-id="{{$gpu->id}}" data-original-title="Edit"><i class="far fa-edit"></i></a></td>
                            </tr>
                            <?php $sl++ ?>
                        @endforeach
                      </form>
...
<script type="text/javascript">
    ...
    $(function () {
      ...
      $('body').on('click', '.editProduct', function () {
        ...
        $.get("{{ url('/model/get/gpu/data') }}" +'/'+ product_id , function (data) {
