        ...
        <div class="col-md-12">
            <div class="card mt-3">
                <div class="card-header bg-info text-white rounded">
                    <div class="row">
                        <div class="col-md-4">
                            <h5><i class="fas fa-suitcase"></i> View All Casings</h5>
                        </div>
                ...
                <div class="card-body">
                    <table class="table table-striped">
                        ...
                        <tbody>
                        <?php $sl = 1 ?>
                        <form action="{{url('casing/price/stock/update')}}" method="POST">
                            @csrf
                            <div class="row">
                                <div class="col-md-10">
                                    {{ $casings->links() }}
                        ...
                        <input type="hidden" name="casing_id[]" value="{{$casing->id}}">
                            <tr>
                                <th scope="row">{{$sl}}</th>
                                <td>{{$casing->brand}}</td>
                                <td><img src="{{url($casing->image)}}" style="height:50px;width:60px"></td>
                                <td>{{$casing->id}}</td>
                                <td>{{$casing->name}}</td>
                                <td>{{$casing->form_factor_name}}</td>
                                @if($casing->stock < 3)
                                <td><input min="0" type="number" placeholder="Stock" name="stock[{{$casing->id}}]" class="text-center form-control" value="{{$casing->stock}}" style="color:red;font-weight:500" required></td>
                                @else
                                <td><input min="0" type="number" placeholder="Stock" name="stock[{{$casing->id}}]" class="text-center form-control" value="{{$casing->stock}}" style="color:green;font-weight:500" required></td>
                                @endif
                                <td><input min="0" type="number" placeholder="Price" name="price[{{$casing->id}}]" class="text-center form-control" value="{{$casing->price}}" required></td>
                                <td>
                                    <a href="{{url('delete/casing')}}/{{$casing->id}}" class="btn btn-sm btn-danger rounded mb-1"><i class="fas fa-trash-alt"></i></a>
                                </td>
                                <td><a class="edit btn btn-warning btn-sm editProduct rounded" href="javascript:void(0)" data-toggle="tooltip" data-id="{{$casing->id}}" data-original-title="Edit"><i class="far fa-edit"></i></a></td>
                            </tr>
                            <?php $sl++ ?>
                        @endforeach
                      </form>
...
<script type="text/javascript">
    ...
    $(function () {
      ...
      $('body').on('click', '.editProduct', function () {
        ...
        $.get("{{ url('/model/get/casing/data') }}" +'/'+ product_id , function (data) {
